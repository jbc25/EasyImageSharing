package es.triskelapps.easyimagesharing.util;
...
public class BitmapUtils {
    ...
    public static Bitmap getRotatedBitmap(String imagePath) {
        ...
        try {
            ...
            // -- Scaling --//
            Bitmap bm1 = decodeSampledBitmap(imagePath, WIDTH_OUTPUT,
                    HEIGHT_OUTPUT);
            float aspectRatio = (float) bm1.getWidth() /
                    (float) bm1.getHeight();
            float scaleX = (float) WIDTH_OUTPUT / (float) bm1.getWidth();
            float scaleY = ((float) WIDTH_OUTPUT / aspectRatio) /
                    (float) bm1.getHeight();

            Matrix matrix1 = new Matrix();
            matrix1.postRotate(rotation1);

            if (bm1.getWidth() > WIDTH_OUTPUT)
